...
def app():
    ...
    if function_option == "Twitter":
        ...
        if st.button("Analyse Sentiment"):
            ...
            if mention.empty:
                st.info("No mentions found in the tweets.")
            ...
            with col4:
                ...
                top_links = data["links"].value_counts().head(10).reset_index()
                top_links.columns = ["link", "count"]  # rename for clarity
                filtered_data = data[data["links"].isin(top_links["link"].values)]
